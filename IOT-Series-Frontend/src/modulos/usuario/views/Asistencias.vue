...
import { obtenerAsistencias } from "../services/UsuarioService"; // Ajusta la ruta si es necesario
...
const socket = io("http://localhost:3000"); // URL de tu backend Node.js
...
// 🔥 NUEVO: Para saber qué valor graficar en el eje Y del Estudiante
...
// 🔥 NUEVO: Configuración para 3 niveles (0, 1, 2)
...
const cargarDatosHistoricos = async () => {
  try {
    const res = await obtenerAsistencias();
    ...
    asistencias.forEach((a) => {
      ...
      // 🔥 Usamos la nueva función para mapear a 0, 1 o 2
    ...
    // 🔥 Inicializar Gráfica: Estudiante (Usando la nueva configuración)
...
const agregarNuevaAsistencia = (nueva) => {
  ...
  // 🔥 Mapeamos el nuevo registro a 0, 1 o 2
...
onMounted(async () => {
  ...
  socket.on("nuevaAsistencia", (nueva) => {
    console.log("Nueva asistencia en tiempo real:", nueva);
    agregarNuevaAsistencia(nueva);
  });
});

onBeforeUnmount(() => {
  socket.off("nuevaAsistencia");
  socket.disconnect();
});
</script>
...
<template>
  <div class="panel-asistencias">
    <h2 class="titulo-panel">Panel de Control de Asistencias</h2>
